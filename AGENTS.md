...
- **[书源规则/JSON 路径] `<js>` 规则的 `result` 必须传真实 JSON 对象，严禁 `toString()`**：JsonPath（Jayway）返回的是 `net.minidev.json.JSONObject`（继承 `HashMap`），`json.toString()` 得到的是 **Java Map 格式** `{book_id=xxx, source=yyy}` 而**不是 JSON**，书源 JS 里 `result.book_id` 恒为 `undefined`，导致 `bookUrl` 生成失败、条目被 `mapNotNull` 丢弃（实测表现为「搜索 0 条」）。必须传入原始 Map 交由沙箱 `toJsValue` 递归转成 `NativeObject`。历史根因见 [`SESSION-HIST-009`](file:///root/legado-server/docs/sessions/SESSION-HIST-009-origin-and-aggregate-source-compat.md)。
- **[书源规则/`##` 语法] `##正则##替换` 必须在 JSON 与 HTML **两条**取值路径统一应用**：`ruleSearch.name = "$.book_name##（别名：.*?）"` 这类规则若只在 HTML 路径（`valuePlain`）处理 `##`，JSON 路径（`valueJson`）会把 `##…` 当作 JsonPath 的一部分 ⇒ 解析异常被 `runCatching` 吞掉 ⇒ 取值恒为 null ⇒ **204 条结果全被丢弃**。统一抽出 `applyRegexReplace(value, rule)`（Legado 语义：`##正则##替换` 成对出现，替换串缺省为空串），两条路径共用。
- **[书源规则/`<js>` 列表规则] `nodes()` 除 JsonPath/CSS 外必须支持 `<js>` 列表规则**：`ruleToc.chapterList` 可能是 `<js>…</js>$.data` 形式的规则链，而 `nodes(body, rule)` 原本只判断「以 `$` 开头 → JsonPath，否则 → CSS 选择器」，会把 `<js>` 当 CSS 选择器 ⇒ **目录 0 章**。同时 `<js>` 之后可以接取值路径（`</js>$.data`）或后置规则，必须按规则链求值而非当作路径字符串。
- **[HTTP/URL 宽容度] `java.net.URI` 严格 vs OkHttp 宽松：query 内的 `{}"` 必须百分比编码**：Legado 书源把**未编码 JSON 直接塞进 URL query**（如 `&variable={"custom":""}`）是常态；Android 侧 OkHttp 会自动编码，而 `java.net.URI(...)` 严格遵循 RFC 3986 抛 `URISyntaxException`（表现为 `URL 解析失败: Illegal character in query`）。`RuleRunner.parseUri` 必须对 query/fragment 非法字符做 `percentEncodeIllegal` 宽容编码——**这是「通杀型」修复**，不修会持续以「莫名其妙的请求失败」形式出现。
- **[沙箱/API 补全] 缺失的沙箱 API 严禁「张冠李戴」补别名，会静默篡改书源控制流**：为凑齐 API 面把 `java.qread()` 别名到了浏览器函数（不抛错），而书源 `checkEnv()` 正是用 **`try { java.qread() } catch`** 来判定客户端环境 ⇒ 分支恒走「轻阅读」⇒ `book.imageStyle='TEXT'` ⇒ **整条正文取数分支走错**。缺失 API 应当「如实缺失（抛错）」或「语义等价地实现」，绝不能拿另一个功能顶替。此类 bug 比直接报错更难查。
- **[沙箱/定位] `login-action` 端点是可执行任意 JS 的通用探针**：`/debug` 类端点只回计数、看不到中间值，而 `login-action` 会把 `toastMessages` 随响应返回 ⇒ 用 `action = "java.toast(String(java.ajax('…')).substring(0,200))"` 即可**在真实书源上下文中打印任意表达式**，无需改代码、无需重启。排障「上游 curl 正常但服务端失败」类问题时，先在沙箱内复现同一请求，即可把问题锁定在本项目解析层而非网络/上游。
- **[工具/PowerShell] `Invoke-WebRequest` 下载大文件比流式慢约 500 倍**：实测同一代理下 IWR 仅 **4~11 KB/s**（会缓冲进内存），而 `HttpWebRequest` 流式下载可达 **~5.2 MB/s**。27.6 MB 的 `legado-server.jar` 用 IWR 需约 2 小时、流式仅需数秒。**大文件一律走流式下载**，GitHub 镜像站（如 `ghfast.top`）实测 11 KB/s 亦不可靠。
- **[工具/PowerShell] 写文件 BOM 是本项目高频陷阱（`.cmd` 与 `plugin.json` 同源）**：Windows PowerShell 5.1 的 `Set-Content -Encoding UTF8` 会写入 BOM，`plugin.json` 开头多出 `\uFEFF` 导致插件全部卸载；批处理 `.cmd` 带 BOM 则表现为首行报 `'锘緻echo' 不是内部或外部命令`（`锘緻` 即 UTF-8 BOM 被按 GBK 解码的结果）。另外用 `Invoke-RestMethod -Body <string>` 传含中文的 JSON 会编码错乱（书源名写坏、后续按名查询 404），联调脚本须显式传 UTF-8 字节。
- **[Rhino 沙箱] JS 桥接抛出的异常必须经 `normalizeScriptThrowable` 归一化，否则抛 CCE 并击穿书源容错**：Kotlin **没有受检异常检查**，桥接函数会把 `SSLHandshakeException`（`extends SSLException extends IOException`，受检）原样放行，而 Rhino 假定逃逸异常是 `java.lang.Error` 系并做 `(Error)` 强转 ⇒ 抛 `class javax.net.ssl.SSLHandshakeException cannot be cast to class java.lang.Error`。**危害不止报错难看**：该 CCE 在 JS 引擎内部抛出，**书源自己的 `try{...}catch(e){}` 抓不到**，聚合源的多节点轮询与主备线路回退被**整体击穿**。实测（Rhino 1.8.0，生产沙箱 `initSafeStandardObjects` + `ClassShutter{false}`）四种形态对比：
  | 抛出形式 | JS 可 catch | `e.name` | 消息 |
  | :--- | :--- | :--- | :--- |
  | 受检异常直接冒泡 | ❌ 抛 CCE | — | — |
  | `RuntimeException` / `Error` 包装 | ❌ **仍漏过** | — | — |
  | `Context.throwAsScriptRuntimeEx` | ✅ | `InternalError` | **丢失真实类型名** |
  | **`ScriptRuntime.constructError("Error", detail)`** | ✅ | `Error` | ✅ 保留真实类型名 |
  ⇒ 必须用 `constructError`。**但 `RuleExecutionException` 要原样透传不包裹**——它是业务语义异常（「未配置 searchUrl」「上游 HTTP 404」），必须冒泡到路由层转成明确提示；包裹后会被书源 catch 当成线路故障静默吞掉。回归测试：`ScriptThrowableNormalizationTest`（6 用例，含 8 种异常 × 3 桥接方法矩阵）。详见 [`SESSION-023`](file:///root/legado-server/docs/sessions/SESSION-023-ssl-handshake-cce.md)。
- **[Rhino 沙箱] 同名属性 `putProperty` 会静默覆盖，审查桥接对象必须查重**：`createJavaBridge` 内先定义 `java.get(url, headers)`（HTTP GET），函数末尾又以 `putProperty(api, "get", ...)` 覆盖为 session store 读取 ⇒ **HTTP GET 长期是死代码且无人发现**（`NativeObject` 重复赋值不报警）。定位手法：异常矩阵中 `java.ajax`/`java.post` 均正常、唯独 `java.get` 行为异常，与「代码看起来没问题」矛盾 ⇒ 逐个 `putProperty(api,"get")` 查重。修法：合并为单一定义按参数形态分派。**凡是往同一桥接对象赋同名属性的地方，都要当成潜在覆盖来审**。
- **[序列化/桥接] `Map<String, Any>` 陷阱不止在 Ktor 路由，JS 桥接内部拼接选项同样是雷区**：`mapOf("method" to "POST", "body" to body, "headers" to headers)` 的静态类型是 `Map<String, Any>`，`Json.encodeToString(opt)` 抛 `SerializationException: Serializer for class 'Any' is not found`。**加重情节**：该语句常在归一化 `try/catch` **之外** ⇒ 桥接**静默返回 null**，用户毫无提示（表现为 `java.post` 完全没反应）。修法：用 `buildJsonObject { put(...) }` 显式构造强类型 `JsonObject`（见 `buildUrlOptionsJson`）。
- **[备份导入/封面] 外部备份只带 `coverUrl`、不带图片本体，必须主动物化 + 查询透出 + 前端回退「三层同修」**：Legado 备份包的 `bookshelf.json` 只有封面 URL。若只做其中一层都会残废——① 导入器不补抓 ⇒ `cover_key` 永久为空；② 书架查询不 `SELECT cover_url` ⇒ URL 也到不了接口（**表里有 ≠ 接口能拿到**，`listBookshelf`/`getShelfBookByUrl`/`getBookshelf` 三处都要查）；③ 前端只认 `coverKey` ⇒ 只能显示文字占位符。**正确做法**：`BackupImporter.backfillCovers()` 并发补抓成本地副本（**必须有上限与静默失败**，图床 404/防盗链是常态，绝不能影响导入本身）＋ 查询透出 `coverUrl` ＋ 前端 `resolveShelfCover()` 优先本地 key、回退 URL。**只做前端 URL 回退是错的**：那会让封面完全依赖外部图床，图床下线或离线阅读时就全瞎。详见 [`SESSION-024`](file:///root/legado-server/docs/sessions/SESSION-024-backup-import-cover-missing.md)。
- **[排障/方法论] 「先查数据，再读代码」**：定位「导入后 XX 显示不出来」这类问题，第一步应是**统计真实库里的字段分布**（如 `total=435 / cover_key 空=434 / cover_url 有值=429 / cover_cache=1`），四个数字即可锁定全貌，远快于在代码里翻找。**同理：迁移/导入类功能的通用自问是「这份备份里有哪些字段需要我们主动补全或物化？」**
- **[启动/缓存] 「续做」函数必须有过滤条件，且并发限流要加在全局而非单任务内部**：`Database.cacheRequests()` 旧实现是 `select ... from book_shelf`（**无 where**），每次启动都把整架书当待缓存任务重下；而 `CACHE_CONCURRENCY = 4` 只是**单本书内部**的并发，同时缓存 N 本时实际并发 = **N × 4** ⇒ 418 本续做时上千请求争抢 `Dispatchers.IO`，服务显示 `Application started` 却**持续超时 4 分钟以上**、数据库从 13MB 膨胀到 343MB。修法：① `cacheRequests()` 只返回 `state in ('pending','caching')` 且**跳过 `content://`/`local://`**（Android/本地路径在服务端必然失败，只会每次启动刷屏）；② 在真正抓取点加**全局** `Semaphore(MAX_GLOBAL_CHAPTER_FETCHES=8)`；③ 续做分批错峰投放。修复后 `/healthz` 从「>240s 常驻超时」变为 **5s 可响应**，`content://` 刷屏 **0 行**。**通用教训：限流加在单个任务内部，当任务数量本身无界时等于没限流。** 详见 [`SESSION-025`](file:///root/legado-server/docs/sessions/SESSION-025-startup-cache-saturation.md)。
- **[排障] `INFO` 级刷屏不等于崩溃，但也绝不能忽略——必须实际探测接口**：用户报「启动报错」，实际是「启动成功但完全不可用」（日志全为 INFO，无异常栈）。只凭日志级别判断会误判为「没问题」。**正确姿势：不管日志怎么说，直接打 `/healthz` 实测响应。**
- **[排障] 验证外部资源可达性必须用系统里真实存储的完整值**：实测中我用手动截断的封面 URL 请求得到 **403**，差点误判为「图床防盗链、浏览器也加载不了、URL 回退无效」；改用**从数据库取出的完整 URL** 重测，全部 **HTTP 200（约 1MB PNG）**。**手敲/截断的样本会把「自己构造错了」误判成「外部服务拒绝」**，进而导向错误的设计改动。
- **[数据修复] 修复「新数据不再出错」≠「老数据自动好了」**：为历史坏数据提供**幂等的一次性修复入口**（如 `POST /api/bookshelf/refresh-covers`，与既有 `/bookshelf/reclean` 语义对齐）。实测用户 428 本待补抓 → 成功 **390**、失败 38（图床链接失效属正常，前端 `coverUrl` 兜底）。
...
| 2026-09-12 | Hist | **奠基会话归档**：裸机装 Git/工具链 → 克隆仓库 → 聚合书源（大灰狼）全链路兼容攻关（`JavaImporter`/8192 请求行/搜索 0 条的三重根因/`data:` 载荷/URI 宽容编码/`qread` 别名污染）＋ 内置 WebView 登录反向代理 ＋ 回归判定方法论起源 | [`docs/sessions/SESSION-HIST-009-origin-and-aggregate-source-compat.md`](file:///root/legado-server/docs/sessions/SESSION-HIST-009-origin-and-aggregate-source-compat.md) | Archived |
...
| 2026-09-26 | Init | `/doc-init` 增量（第 2 次）：全量重扫 DSH 会话（28 份 / 含跨工作区），补齐**缺失的奠基会话**（`yuedu web` 工作区 10,079 帧 / 29 轮诉求 / 452 次工具调用），新增 HIST-009 归档并补录 8 条部落知识（JSON `[]` 传参/`##` 双路径/`<js>` 列表规则/URI 宽容编码/沙箱 API 别名禁忌/login-action 探针/流式下载/BOM 陷阱） | [`docs/sessions/SESSION-HIST-009-origin-and-aggregate-source-compat.md`](file:///root/legado-server/docs/sessions/SESSION-HIST-009-origin-and-aggregate-source-compat.md) | Local（未推送） |
| 2026-09-26 | Fix | 修复大灰狼书源登录报错 `SSLHandshakeException cannot be cast to java.lang.Error`：新增 `normalizeScriptThrowable` 归一化 JS 桥接异常（恢复书源 catch 容错能力）；**对抗审查连带挖出另外两处缺陷**——`java.post/get` 的 `Map<String,Any>` 序列化静默返回 null、`java.get` 被同名属性顶掉导致 HTTP GET 是死代码。新增 `ScriptThrowableNormalizationTest`（6 用例）；基线失败集合逐条比对零回归（199→205 用例，53 失败集合一致） | [`docs/sessions/SESSION-023-ssl-handshake-cce.md`](file:///root/legado-server/docs/sessions/SESSION-023-ssl-handshake-cce.md) | Tested |
| 2026-09-26 | Fix | 修复「从备份导入的书封面加载不出来」**三层根因**：① 导入器只存 `cover_url` 从不下载（`cover_key` 永久为空）；② 三个书架查询都没 `SELECT cover_url`（URL 也到不了接口）；③ 前端只认 `coverKey` 无 URL 回退。新增 `backfillCovers()` 并发补抓（上限 300/6 并发/静默失败）＋ 查询透出 `coverUrl` ＋ `resolveShelfCover()`。实测用户 435 本书中 429 本恢复封面来源；新增 3 回归用例，零回归（199→208 用例，53 失败集合一致） | [`docs/sessions/SESSION-024-backup-import-cover-missing.md`](file:///root/legado-server/docs/sessions/SESSION-024-backup-cover-missing.md) | Tested |
| 2026-09-26 | Fix | 修复「源码启动后服务不可用」：① `cacheRequests()` 无 where 导致每次启动重下整架书（改为只续做 `pending`/`caching` 并跳过 `content://`/`local://`）；② `CACHE_CONCURRENCY` 只是单书内并发，多书并行时 = N×4 上千请求打满线程池（新增**全局** `Semaphore(8)`）；③ 续做分批错峰。`/healthz` 从「>240s 常驻超时」→ **5s 可响应**。另新增 `POST /api/bookshelf/refresh-covers` 修复历史数据，实测 428 本待补抓 → **成功 390**，`coverKey` 从 0 → 391 | [`docs/sessions/SESSION-025-startup-cache-saturation.md`](file:///root/legado-server/docs/sessions/SESSION-025-startup-cache-saturation.md) | Tested |
