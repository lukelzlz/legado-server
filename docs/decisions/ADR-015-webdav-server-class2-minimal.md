...
# ADR-015: 进程内 WebDAV 服务端选型、Basic 鉴权与最小 Class 2 锁实现

## 1. 决策背景 (Context)

PROPOSAL-015 要求在既有 Ktor 服务端进程内提供 WebDAV 能力，并把上传数据落在数据目录下的 `webdav/` 文件夹。围绕「用什么实现、怎么鉴权、锁怎么做、路径怎么解析」有若干互斥方案，需一次性裁定，避免后续反复重构。
