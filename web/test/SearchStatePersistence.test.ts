...
test('SearchStatePersistence - chooseSource synthesizes cover and intro across alternate sources', async () => {
  const store = new SearchStore()
  const candidateResults: SearchResult[] = [
    { sourceId: 'source-no-cover', bookUrl: 'http://s1/book', name: '【精品】诡秘之主', author: '爱潜水的乌贼' },
    { sourceId: 'source-with-cover', bookUrl: 'http://s2/book', name: '诡秘之主', author: '爱潜水的乌贼', coverUrl: 'http://example.com/guimi.jpg', intro: '蒸汽朋克与克苏鲁神话' },
  ]
  ...
  store.setChoices([
    {
      ...
      book: {
        details: {
          sourceId: 'source-no-cover',
          name: '【精品】诡秘之主',
          author: '爱潜水的乌贼',
          tocUrl: 'http://s1/toc',
        },
        bookUrl: 'http://s1/book',
        chapters: [{ index: 0, title: '第一章 绯红', url: 'http://s1/c1' }],
      },
    },
    {
  ...
  // Choose the source without a cover
  await store.chooseSource(store.getSnapshot().choices[0], candidateResults)

  const openBook = store.getSnapshot().openBook
  assert.ok(openBook, 'openBook should be populated')
  assert.equal(openBook!.details.coverUrl, 'http://example.com/guimi.jpg', 'should synthesize cover from alternate candidate')
  assert.equal(openBook!.details.intro, '蒸汽朋克与克苏鲁神话', 'should synthesize intro from alternate candidate')
  assert.equal(openBook!.details.alternateSources?.length, 1, 'alternateSources should contain the other source')
  assert.equal(openBook!.details.alternateSources?.[0].sourceId, 'source-with-cover')
})
